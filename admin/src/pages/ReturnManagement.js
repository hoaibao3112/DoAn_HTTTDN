...
const ReturnManagement = () => {
  ...
  const fetchReturns = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${API}/api/returns`, { params });
  ...
  const openDetail = async (id) => {
    try {
      setProcessing(true);
      const { data } = await axios.get(`${API}/api/returns/${id}`);
  ...
  const handleAction = async (id, action, opts = {}) => {
    ...
    try {
      ...
      const { data } = await axios.put(`${API}/api/returns/${id}/action`, payload);
