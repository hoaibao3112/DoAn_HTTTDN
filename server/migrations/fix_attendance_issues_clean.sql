...
-- 1. Thêm cột PhutNghi vào bảng ca_lam_viec
DELIMITER $$
...
-- 4. Thêm các cột tracking vào bảng cham_cong
DELIMITER $$
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS 
        WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cham_cong' AND COLUMN_NAME = 'DiaChi_IP'
    ) THEN
        ALTER TABLE cham_cong ADD COLUMN DiaChi_IP VARCHAR(50) COMMENT 'IP địa chỉ chấm công';
    END IF;
    
    -- Thêm ViTri
    IF NOT EXISTS (
        SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS 
        WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cham_cong' AND COLUMN_NAME = 'ViTri'
    ) THEN
        ALTER TABLE cham_cong ADD COLUMN ViTri VARCHAR(255) COMMENT 'Tọa độ GPS (nếu có)';
    END IF;
...
SET @sql_idx_ngay := IF(@exist_idx_ngay = 0, 
    ...
    'SELECT "Index idx_ngay already exists" AS message'
);
...
SET @sql_idx_trangthai := IF(@exist_idx_trangthai = 0, 
    ...
    'SELECT "Index idx_trangthai already exists" AS message'
);
...
SET @sql_idx_manv_ngay := IF(@exist_idx_manv_ngay = 0, 
    ...
    'SELECT "Index idx_manv_ngay already exists" AS message'
);
PREPARE stmt FROM @sql_idx_manv_ngay;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 6. Tạo view báo cáo chấm công bất thường
DROP VIEW IF EXISTS v_cham_cong_bat_thuong;
CREATE VIEW v_cham_cong_bat_thuong AS
SELECT 
    nv.MaNV,
    nv.HoTen,
    YEAR(cc.Ngay) as Nam,
...
-- 7. Tạo stored procedure tự động đánh vắng mặt
DELIMITER $$

DROP PROCEDURE IF EXISTS sp_auto_mark_absent$$
CREATE PROCEDURE sp_auto_mark_absent(IN target_date DATE)
BEGIN
...
-- 8. Tạo trigger ghi log khi sửa chấm công
DELIMITER $$

DROP TRIGGER IF EXISTS tr_cham_cong_before_update$$
CREATE TRIGGER tr_cham_cong_before_update
...
-- HƯỚNG DẪN SỬ DỤNG
-- =============================================
-- 1. Chạy toàn bộ script này trong MySQL Workbench
-- 2. Để test stored procedure tự động đánh vắng mặt:
--    CALL sp_auto_mark_absent('2026-02-03');
-- 3. Để xem báo cáo chấm công bất thường:
--    SELECT * FROM v_cham_cong_bat_thuong WHERE Nam = 2026 AND Thang = 2;
-- =============================================
